...
- order      : 메뉴 탐색, 메뉴 문의(있는지/가격/구성/맵기/양 등 "삼겹솥밥 있나", "얼마예요", "뭐 들어가요"),
               ...
               장바구니 담기/수정/삭제, 장바구니 초기화/비우기("처음부터", "다시 할게요", "취소", "비워줘", "다 지워줘", "싹 지워"),
               옵션/추가 항목 거절("계란 추가 안 할게", "옵션 없이", "그냥 줘", "기본으로")
- payment    : 결제 요청, 주문 확정/주문 확인 완료/확인 완료, 결제 수단 선택("카드로", "정맥으로", "결제할게요", "주문할게요"),
               더 이상 주문할 메뉴가 없다는 명확한 의사 표현("없어요", "됐어요", "그게 다야", "다 됐어요", "이게 다야")
- recommend  : 추천 요청, 뭐가 맛있냐, 인기 메뉴가 뭐냐, 조건 기반 추천 요청,
               날씨·기온 언급("더워", "추워", "비 와", "날씨 어때", "더운데", "추운데")으로 메뉴를 찾는 발화
               (날씨에 맞는 시원한/뜨끈한 메뉴 추천으로 연결한다)
- hesitation : 망설임, 침묵 후 재개, "음...", "뭐가 좋지", 결정 못 하는 발화
- clarify    : 음식·메뉴·주문·결제와 "전혀" 무관한 발화에만 쓴다 (시간/뉴스/길찾기/잡담/욕설/광고),
               단순 인사("안녕", "안녕하세요", "처음이야")

판단 기준 (우선순위 순):
- 음식·메뉴 이름이 언급되거나 "있나/있어요/파나요/얼마예요/뭐 들어가요/매워요" 같은 메뉴 문의는
  우리 매장에 없는 메뉴여도 반드시 order 로 분류한다. 절대 clarify 가 아니다.
- "치즈라면 어때?", "가츠동 괜찮아?", "냉모밀 어떤지" 처럼 '특정 메뉴 이름 1개'를 콕 집어 묻는 발화는
  recommend 가 아니라 order(그 메뉴 문의)로 분류한다. recommend 는 메뉴 이름 없이 "뭐 맛있어?/추천해줘/인기 메뉴" 처럼
  열린 추천 요청일 때만 쓴다.
- "카드", "정맥", "결제" 단어가 포함된 발화는 반드시 payment 로 분류한다.
- "주문 확인 완료", "주문 확정", "확인 완료", "주문 완료" 처럼 주문을 확정해 다음 단계로 넘어가려는 발화는 payment 로 분류한다 (clarify 아님).
...
- "더워/덥다/추워/춥다/비 와/날씨" 같은 날씨·기온 발화는 시원한/뜨끈한 메뉴 추천으로 이어지므로 recommend 로 분류한다 (clarify 아님).
